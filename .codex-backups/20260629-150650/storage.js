(function () {
  ...
  const sampleState = () => {
    ...
    return {
      ...
      clients: [],
      pinnedClientIds: [],
      tags: ["WhatsApp", "Lead", "Cliente", "Parceiro"],
      futureTools: {
        scheduledMessages: [],
        quickScripts: [],
        aiSummaries: [],
  ...
  const normalizeNote = (note) => ({
    id: note.id || uid("note"),
    text: clean(note.text || note.note),
  ...
  const normalizeClient = (client, state) => {
    ...
    return {
      ...
      waKey: clean(client.waKey) || clean(client.phone) || clean(client.chatTitle) || clean(client.name) || uid("wa"),
      label: clean(client.label || tags[0]),
      tags,
      temperature: ["frio", "morno", "quente"].includes(client.temperature) ? client.temperature : "morno",
      value: toNumber(client.value),
  ...
  const normalizeState = (input) => {
    ...
    const pinnedFromClients = state.clients.filter((client) => client.pinned).map((client) => client.id);
    state.pinnedClientIds = Array.from(new Set([...state.pinnedClientIds, ...pinnedFromClients])).filter((id) =>
      state.clients.some((client) => client.id === id)
    );
    state.clients.forEach((client) => { client.pinned = state.pinnedClientIds.includes(client.id); });

    const tagsFromClients = state.clients.flatMap((client) => client.tags || []);
    state.tags = Array.from(new Set([...state.tags, ...tagsFromClients, "WhatsApp", "Lead"].map(clean).filter(Boolean)));
    return state;
  };
  ...
  const createFunnel = (name) => updateState((state) => {
  ...
  const createColumn = (funnelId, name) => updateState((state) => {
  ...
  const renameColumn = (funnelId, columnId, name) => updateState((state) => {
    const funnel = state.funnels.find((item) => item.id === funnelId);
  ...
  const deleteColumn = (funnelId, columnId) => updateState((state) => {
    const funnel = state.funnels.find((item) => item.id === funnelId);
  ...
  const upsertClient = (client) => updateState((state) => {
    const stamp = now();
    const incomingKey = clean(client.waKey) || clean(client.phone) || clean(client.chatTitle) || clean(client.name);
    const index = state.clients.findIndex((item) => {
      const sameId = client.id && item.id === client.id;
      const samePhone = client.phone && item.phone && clean(item.phone) === clean(client.phone);
      const sameKeyInFunnel = incomingKey && item.waKey === incomingKey && item.funnelId === client.funnelId;
      return sameId || samePhone || sameKeyInFunnel;
    });
  ...
  const deleteClient = (clientId) => updateState((state) => {
    state.clients = state.clients.filter((client) => client.id !== clientId);
    state.pinnedClientIds = state.pinnedClientIds.filter((id) => id !== clientId);
    return state;
  });

  const moveClient = (clientId, columnId) => updateState((state) => {
  ...
  const addNote = (clientId, text) => updateState((state) => {
    const client = state.clients.find((item) => item.id === clientId);
  ...
  const addReminder = (clientId, reminder) => updateState((state) => {
    const client = state.clients.find((item) => item.id === clientId);
    const title = clean(reminder && (reminder.title || reminder.text));
    if (client && title) {
      client.reminders.unshift({ id: uid("reminder"), title, dueAt: clean(reminder.dueAt), note: clean(reminder.note), done: false, createdAt: now() });
      client.nextAction = client.nextAction || title;
      client.updatedAt = now();
    }
    return state;
  });

  const togglePinned = (clientId) => updateState((state) => {
    const client = state.clients.find((item) => item.id === clientId);
    if (!client) return state;
    const pinned = !state.pinnedClientIds.includes(clientId);
    state.pinnedClientIds = pinned ? [...state.pinnedClientIds, clientId] : state.pinnedClientIds.filter((id) => id !== clientId);
