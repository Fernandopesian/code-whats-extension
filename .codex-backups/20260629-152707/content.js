(function () {
  ...
  function getDetectedChats() {
    ...
    rows.forEach((row, index) => {
      ...
      const titleNode = qs('span[title]', row) || qs('span[dir="auto"]', row);
      const name = clean(titleNode && (titleNode.getAttribute("title") || titleNode.textContent)) || text.split(/\d{1,2}:\d{2}|ontem|segunda|terça|quarta|quinta|sexta|sábado|domingo/i)[0].trim();
      if (!name || name.length > 80) return;
      const img = qs("img", row);
      const photo = img ? clean(img.src) : "";
      const timeMatch = text.match(/\b\d{1,2}:\d{2}\b|ontem|segunda|terça|quarta|quinta|sexta|sábado|domingo/i);
      const lastMessage = text.replace(name, "").replace(timeMatch ? timeMatch[0] : "", "").trim().slice(0, 180);
      const unreadNode = qs('[aria-label*="mensagem"], [aria-label*="message"]', row);
      const unreadText = clean(unreadNode && unreadNode.getAttribute("aria-label"));
      const key = [name, photo ? photo.slice(-32) : "", index].join("|");
      if (seen.has(name.toLowerCase())) return;
      seen.add(name.toLowerCase());
      chats.push({ key, name, chatTitle: name, photo, phone: "", waKey: key, lastMessage, lastMessageTime: timeMatch ? timeMatch[0] : "", unread: unreadText, rowIndex: index });
    });

    return chats.slice(0, 40);
  }
  ...
  function chatMeta(chat) {
    return [chat.phone, chat.lastMessageTime, `${chat.visibleMessageCount || 0} mensagens visiveis`].filter(Boolean).join(" • ");
  }
  ...
  function handleDragEnd(event) { const card = event.target.closest(".cwl-card"); if (card) card.classList.remove("is-dragging"); }
  ...
  function bootObserver() {
    observer = new MutationObserver(() => { clearTimeout(scanTimer); scanTimer = setTimeout(scanWhatsApp, 500); });
    observer.observe(document.body, { childList: true, subtree: true, characterData: true });
    setInterval(scanWhatsApp, 3500);
  }
