(function () {
        ...
        ${funnel.columns.map((column) => `
          <article class="cwl-crm-column" data-column-id="${column.id}">
            ...
            <div class="cwl-crm-dropzone" data-column-id="${column.id}">
              ${clientsForColumn(column.id).map(renderCard).join("") || `<p class="cwl-empty">Sem clientes.</p>`}
            </div>
          </article>
        `).join("")}
      </section>
  ...
  function renderCard(client) {
    return `
      <article class="cwl-card" draggable="true" data-client-id="${client.id}">
        ...
        <div class="cwl-card-actions"><button data-action="open-whatsapp" data-client-id="${client.id}">Abrir conversa</button></div>
      </article>
    `;
  }
  ...
  async function openInWhatsApp(clientId) {
    ...
    if (!tabs.length) {
      ...
      alert("Abra o WhatsApp Web, recarregue a extensao e tente novamente.");
      return;
    }
  ...
  document.addEventListener("dragstart", (event) => {
    ...
    if (!card) return;
    event.dataTransfer.setData("application/cwl-client", card.dataset.clientId);
