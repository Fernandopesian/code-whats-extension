(function () {
  ...
  function renderView() { if (activeView === "crm") return renderCrmShortcut(); if (activeView === "notes") return renderNotesView(); if (activeView === "backup") return renderBackup(); if (activeView === "sync") return renderSyncPanel(); return renderClients(); }
  function renderSyncPanel() { const pending = (state.syncEvents || []).filter((event) => event.syncStatus === "pending").length; const status = state.syncStatus || "offline"; return `<section class="cwl-card-panel"><h2>Sincronização</h2><p>Status: <strong>${esc(status)}</strong></p><p>Eventos pendentes: ${pending}</p><p class="cwl-empty">Arquitetura preparada para Supabase Auth, Realtime, Edge Functions, Storage e RLS. Nenhuma API real esta conectada nesta etapa.</p><div class="cwl-inline-actions"><button class="cwl-btn cwl-btn-primary" data-action="connect-code">Preparar conexão</button><button class="cwl-btn cwl-btn-ghost" data-action="push-pending-events">Enviar eventos pendentes</button><button class="cwl-btn cwl-btn-ghost" data-action="disconnect-code">Ficar offline</button></div></section>`; }
  ...
  async function handleClick(event) {
    ...
    if (action === "set-view") { if (target.dataset.view === "crm") { chrome.runtime.sendMessage({ type: "CWL_OPEN_CRM_WINDOW" }); return; } activeView = target.dataset.view; await saveUi(); render(); return; }
    ...
    if (action === "connect-code") { state = await storage.connectCode(); render(); return; }
    if (action === "push-pending-events") { state = await storage.pushPendingEvents(); render(); return; }
    if (action === "disconnect-code") { state = await storage.disconnectCode(); render(); return; }
